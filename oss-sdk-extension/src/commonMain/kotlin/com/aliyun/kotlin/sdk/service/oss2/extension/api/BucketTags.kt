...
public suspend fun OSSClient.putBucketTags(request: PutBucketTagsRequest, options: OperationOptions? = null): PutBucketTagsResult {
    requireNotNull(request.bucket) { "request.bucket is required" }
    requireNotNull(request.tagging) { "request.tagging is required" }
    ...
    val input = OperationInput {
        ...
        // body
        body = SerdeUtils.serializeXmlBody(request.tagging).toByteStream()
    }
...
public suspend fun OSSClient.getBucketTags(request: GetBucketTagsRequest, options: OperationOptions? = null): GetBucketTagsResult {
    requireNotNull(request.bucket) { "request.bucket is required" }
    ...
    return GetBucketTagsResult {
        headers = output.headers
        status = output.status
        statusCode = output.statusCode
        innerBody = SerdeUtils.deserializeXmlBody<Tagging>(body)
    }
}

public suspend fun OSSClient.deleteBucketTags(request: DeleteBucketTagsRequest, options: OperationOptions? = null): DeleteBucketTagsResult {
    requireNotNull(request.bucket) { "request.bucket is required" }
